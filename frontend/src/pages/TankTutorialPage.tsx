...
type TankTutorialScreen = 'intro' | 'playing' | 'victory' | 'stage_end';
...
export default function TankTutorialPage() {
  ...
  const DIALOGUE_SEQUENCES = useMemo(() => ({
    intro: t('tutorial.tank.intro', { returnObjects: true }) as string[],
    wave1_clear: t('tutorial.tank.wave1', { returnObjects: true }) as string[],
    wave2_clear: t('tutorial.tank.wave2', { returnObjects: true }) as string[],
    wave3_clear: t('tutorial.tank.wave3', { returnObjects: true }) as string[],
    wave4_clear: t('tutorial.tank.wave4', { returnObjects: true }) as string[],
    victory: t('tutorial.tank.victory', { returnObjects: true }) as string[],
  }), [t]);

  const MASTERY = useMemo(() => t('tutorial.tank.mastery', { returnObjects: true }) as Array<{ title: string; desc: string }>, [t]);
  ...
  useEffect(() => {
    ...
    if (engineRef.current) {
      ...
      if (heroes[0]) heroes[0].name = t('tutorial.tank.myName');
      if (heroes[1]) heroes[1].name = t('tutorial.tank.weakHealer');
    }
  }, [t]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setDialogueIdx(i => i + 1), 1200);
  ...
  const handleStartGame = useCallback(() => {
    ...
    const engine = new GameEngine(
      () => { if (engineRef.current) setGameState({ ...engineRef.current.state }); },
      {
        ...
        customHeroes: [
          {
            name: t('tutorial.tank.myName'),
            role: 'tank',
            specName: '분노/방어', className: '전사',
            raceName: '언데드', elementName: '화염',
            maxHp: 3000, hp: 3000,
            atk: 200, def: 50, speed: 3.5,
            attackRange: 60, aggroRadius: 150,
            threatMult: 5.0,
            isAlive: true,
          ...
          {
            name: t('tutorial.tank.weakHealer'),
            role: 'healer',
            specName: '신성', className: '사제',
    ...
    <div className="min-h-screen bg-gray-950 text-white flex flex-col items-center">
      ...
      {screen === 'intro' && (
        <div className="fixed inset-0 z-50 bg-black/90 flex flex-col items-center justify-center p-8">
          <div className="flex items-center gap-6 mb-12">
            <img src="/graphic2/제다/제다right.gif" alt="Jedah" className="w-32 h-32 border-4 border-red-600 rounded-full bg-gray-800" />
            <div>
              <p className="text-red-500 font-bold tracking-widest uppercase">Class Tutorial</p>
              <h1 className="text-5xl font-extrabold text-white">{t('tutorial.tank.pageTitle')}</h1>
            </div>
          </div>

          <div className="max-w-2xl w-full bg-gray-900 border-2 border-red-900/50 rounded-2xl p-8 shadow-2xl shadow-red-900/20">
            <div className="h-[160px] overflow-y-auto text-xl leading-relaxed text-gray-200">
              <span className="text-red-400 font-bold">{t('tutorial.tank.jedahName')}:</span> "{DIALOGUE_SEQUENCES.intro[dialogueIdx]}"
            </div>
      ...
      <div className="relative w-full max-w-5xl mt-4">
        {showCanvas && (
          <div className="flex items-center justify-between px-4 py-2 bg-gray-900 border-b border-red-900/50">
            <div className="flex items-center gap-4">
              <span className="text-red-500 font-bold">{t('tutorial.tank.trainingLabel')}</span>
            ...
            <div className="flex gap-4 items-center">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-500">{t('tutorial.tank.myName')}</span>
                <div className="w-32 h-2 bg-gray-800 rounded-full overflow-hidden border border-gray-700">
                  <div className="h-full bg-red-600" style={{ width: `${((gameState?.heroes[0]?.hp ?? 0) / (gameState?.heroes[0]?.maxHp ?? 3000)) * 100}%` }} />
                </div>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-xs text-gray-500 italic">{t('tutorial.tank.weakHealer')}</span>
                <div className="w-20 h-2 bg-gray-800 rounded-full overflow-hidden border border-gray-700">
                  <div className="h-full bg-green-500" style={{ width: `${(gameState?.heroes[1]?.hp ?? 0) * 100}%` }} />
                </div>
              </div>
            </div>
          </div>
        ...
        {screen === 'victory' && (
          <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center">
            <div className="bg-gray-900 border-4 border-yellow-500 p-8 rounded-3xl text-center max-w-lg animate-bounce-short">
              <h2 className="text-4xl font-black text-yellow-400 mb-6">TRAINING COMPLETE</h2>
              <p className="text-xl text-white italic font-bold">
                {t('tutorial.tank.jedahName')}: "{DIALOGUE_SEQUENCES.victory[dialogueIdx]}"
              </p>
            </div>
          </div>
        )}
      ...
      {screen === 'stage_end' && (
        <div className="fixed inset-0 z-50 bg-gray-950 flex flex-col items-center justify-center p-8 text-center">
          ...
          <h2 className="text-5xl font-black text-blue-400 mb-2">{t('tutorial.tank.masterTitle')}</h2>
          <p className="text-gray-400 text-xl mb-12">{t('tutorial.tank.masterSub')}</p>
