...
import { generateMechanicTutorialWave } from '../game/TutorialWaveData';
...
type MechanicScreen = 'intro' | 'playing' | 'victory' | 'stage_end';

export default function MechanicTutorialPage() {
  ...
  const DIALOGUE_SEQUENCES = useMemo(() => ({
    intro: t('tutorial.mechanic.intro', { returnObjects: true }) as string[],
    wave1_clear: t('tutorial.mechanic.wave1', { returnObjects: true }) as string[],
    wave2_clear: t('tutorial.mechanic.wave2', { returnObjects: true }) as string[],
    wave3_clear: t('tutorial.mechanic.wave3', { returnObjects: true }) as string[],
    wave4_clear: t('tutorial.mechanic.wave4', { returnObjects: true }) as string[],
    victory: t('tutorial.mechanic.victory', { returnObjects: true }) as string[],
  }), [t]);

  const MASTERY = useMemo(() => t('tutorial.mechanic.mastery', { returnObjects: true }) as Array<{ title: string; desc: string }>, [t]);
  const coilzekName = t('tutorial.mechanic.coilzekName');
  const iskierName = t('tutorial.mechanic.iskierName');
  ...
  useEffect(() => {
    ...
    if (engineRef.current) {
      ...
      if (heroes[0]) heroes[0].name = t('tutorial.mechanic.myName');
      if (heroes[1]) heroes[1].name = t('tutorial.mechanic.iskierName');
    }
  }, [t]);
  ...
  const handleStartGame = useCallback(() => {
    ...
    const engine = new GameEngine(
      () => { if (engineRef.current) setGameState({ ...engineRef.current.state }); },
      {
        ...
        customHeroes: [
          {
            name: t('tutorial.mechanic.myName'),
            role: 'mechanic',
            specName: '포탑강화', className: '메카닉',
            raceName: '노움', elementName: '번개',
            maxHp: 900, hp: 900,
            atk: 88, def: 18, speed: 2.8,
            attackRange: 0, aggroRadius: 0,
            threatMult: 1.0,
            isAlive: true,
          ...
          {
            name: t('tutorial.mechanic.iskierName'),
            role: 'ranged_dps',
            specName: '냉기', className: '마법사',
            raceName: '블러드엘프', elementName: '냉기',
            maxHp: 800, hp: 800,
            atk: 280, def: 15, speed: 2.5,
        ...
        waveGenerator: (waveNumber) => {
          const wave = generateMechanicTutorialWave(waveNumber);
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col items-center">
        ...
        <div className="fixed inset-0 z-50 bg-black/90 flex flex-col items-center justify-center p-8">
          <div className="flex items-center gap-8 mb-12">
            <div className="flex -space-x-12">
              <img
                src="/graphic2/이스키에르피리아/이스키에르피리아right.gif"
              ...
              />
              <div className={`w-32 h-32 border-4 rounded-full bg-gray-800 flex items-center justify-center text-5xl transition-all duration-500 ${speakingName === coilzekName ? 'border-cyan-500 scale-110 z-30' : 'border-gray-700 opacity-40 scale-90 z-10'}`}>
                🔧
              </div>
            </div>
            <div>
              <p className="text-cyan-500 font-bold tracking-widest uppercase">Class Tutorial</p>
              <h1 className="text-5xl font-extrabold text-white">{t('tutorial.mechanic.pageTitle')}</h1>
            </div>
          </div>

          <div className="max-w-2xl w-full bg-gray-900 border-2 border-cyan-900/50 rounded-2xl p-8 shadow-2xl shadow-cyan-900/20">
            <div className="min-h-[160px] text-xl leading-relaxed text-gray-200">
              <span className={`font-bold ${speakingName === coilzekName ? 'text-cyan-400' : 'text-blue-400'}`}>
                {speakingName}:
              </span>
            ...
            <div className="mt-8 flex justify-between items-center">
              <div className="flex gap-1">
                {DIALOGUE_SEQUENCES.intro.map((_, i) => (
                  <div key={i} className={`w-2 h-2 rounded-full ${i <= dialogueIdx ? 'bg-cyan-500' : 'bg-gray-700'}`} />
                ))}
              </div>

              {dialogueIdx >= DIALOGUE_SEQUENCES.intro.length - 1 ? (
                <button onClick={handleStartGame} className="px-8 py-3 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-lg transition-transform hover:scale-105">
                  {t('tutorial.mechanic.startBtn')}
                </button>
              ) : (
      ...
      <div className="relative w-full max-w-5xl mt-4">
        {showCanvas && (
          <div className="flex items-center justify-between px-4 py-2 bg-gray-900 border-b border-cyan-900/50">
            <span className="text-cyan-500 font-bold">{t('tutorial.mechanic.trainingLabel')}</span>
            <div className="flex gap-6 items-center">
              <div className="flex gap-1">
                {[1, 2, 3].map(s => (
                  <button key={s} onClick={() => handleSpeed(s)}
                    className={`px-2 py-0.5 text-xs font-bold rounded ${speed === s ? 'bg-cyan-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}>
                    x{s}
                  </button>
                ))}
              </div>
              <div className="flex items-center gap-2">
                ...
                <div className="w-48 h-2 bg-gray-800 rounded-full overflow-hidden border border-gray-700">
                  <div className="h-full bg-orange-600 transition-all duration-300" style={{ width: `${((gameState?.walls[0]?.hp ?? 0) / 25000) * 100}%` }} />
                </div>
              </div>
        ...
        {narration && (
          <div className="absolute bottom-10 left-1/2 -translate-x-1/2 w-full max-w-2xl bg-black/80 border-2 border-cyan-600 rounded-xl p-4 text-center">
        ...
        {screen === 'victory' && (
          <div className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center">
            <div className="bg-gray-900 border-4 border-cyan-500 p-8 rounded-3xl text-center max-w-lg animate-bounce-short">
              <h2 className="text-4xl font-black text-cyan-400 mb-6">MECHANIC COMPLETE</h2>
      ...
      {screen === 'stage_end' && (
        <div className="fixed inset-0 z-50 bg-gray-950 flex flex-col items-center justify-center p-8 text-center">
          ...
          <h2 className="text-5xl font-black text-cyan-500 mb-2">{t('tutorial.mechanic.masterTitle')}</h2>
          <p className="text-gray-400 text-xl mb-12">{t('tutorial.mechanic.masterSub')}</p>
          ...
          <button onClick={goToSelection} className="px-12 py-4 bg-cyan-600 hover:bg-cyan-500 text-white text-xl font-bold rounded-xl transition-all">
